...
async def test_person_profile() -> bool:
    ...
    try:
        ...
        print_info(
            f"Calling: proxycurl.linkedin.person.get(linkedin_profile_url='{linkedin_url}')"
        )
        person = await proxycurl.linkedin.person.get(linkedin_profile_url=linkedin_url)
...
async def test_person_lookup() -> bool:
    ...
    try:
        ...
        print_info(
            "Calling: proxycurl.linkedin.person.resolve(first_name='Bill', last_name='Gates', company_domain='microsoft.com')"
        )
        result = await proxycurl.linkedin.person.resolve(
            first_name="Bill", last_name="Gates", company_domain="microsoft.com"
        )
...
def print_summary(results: Dict[str, bool]):
    ...
    if passed == total:
        ...
        print(
            "   4. Old: proxycurl.linkedin.person.get() → New: enrichlayer.person.get()"
        )
    else:
        failed_tests = [name for name, success in results.items() if not success]
        print(f"⚠️  Some tests failed: {', '.join(failed_tests)}")
        print(
            "   This might be due to API limits, missing dependencies, or network issues"
        )
