...
class TestAllEndpoints(unittest.TestCase):
    ...
    def test_asyncio_core_endpoints(self):
        ...
        asyncio_tests = [
            ...
            ("person.get", lambda: asyncio.run(client.person.get(
                linkedin_profile_url=self.test_data["person_url"], extra="exclude"
            )), 1),
            ("person.search", lambda: asyncio.run(client.person.search(
                country="US", first_name="John", last_name="Smith", enrich_profiles="skip"
            )), 10),
            
            # Company endpoints (sample)
            ("company.get", lambda: asyncio.run(client.company.get(url=self.test_data["company_url"])), 1),
            ("company.find_job", lambda: asyncio.run(client.company.find_job(
                keyword="engineer", geo_id="103644278"
            )), 2),
            
            # School endpoints
            ("school.get", lambda: asyncio.run(client.school.get(url=self.test_data["school_url"])), 1),
            
            # Job endpoints
            ("job.get", lambda: asyncio.run(client.job.get(url=self.test_data["job_url"])), 1),
        ]
        ...
        for endpoint_name, endpoint_func, expected_credits in asyncio_tests:
            with self.subTest(endpoint=endpoint_name, client="asyncio"):
                try:
                    ...
                    self._record_result(f"asyncio.{endpoint_name}", "asyncio", "success", 
                                      expected_credits, sample_data=sample_data)
                    
                except Exception as e:
                    self._record_result(f"asyncio.{endpoint_name}", "asyncio", "error", 0, str(e))
