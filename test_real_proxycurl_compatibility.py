...
async def test_proxycurl_asyncio_compatibility(api_key: str):
    ...
    try:
        ...
        try:
            ...
            print("✅ Monkey patching successful - proxycurl.asyncio is using EnrichLayer backend!")
...
async def test_before_after_comparison(api_key: str):
    ...
    try:
        ...
        modules_to_clear = [m for m in sys.modules.keys() if m.startswith(('proxycurl', 'enrichlayer'))]
...
async def main():
    ...
    tests = [
        ("Proxycurl AsyncIO Compatibility", test_proxycurl_asyncio_compatibility(api_key)),
        ("Proxycurl Gevent Compatibility", test_proxycurl_gevent_compatibility(api_key)),
        ("Proxycurl Twisted Compatibility", test_proxycurl_twisted_compatibility(api_key)),
    ]
